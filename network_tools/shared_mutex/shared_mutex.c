...
shared_mutex_t shared_mutex_init(const char *name, mode_t mode) {
    ...
    // pthread_mutex_t.
    if (ftruncate(mutex.shm_fd, sizeof(pthread_mutex_t)) != 0) { //+sizeof(char)+16
        perror("ftruncate");
        return mutex;
    }
    ...
    volatile char *spin_lock_char = (char *)(addr+sizeof(pthread_mutex_t));

    // If shared memory was just initialized -
    // initialize the mutex as well.
    if (mutex.created) {
        // NOTE ME!
        *spin_lock_char = UNLOCKED;
    ...
    mutex.ptr = mutex_ptr;
    mutex.spin_lock_char = spin_lock_char;
