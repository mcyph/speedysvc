...
class DFileArchive:
    ...
    def __getitem__(self, item):
        seek_pos = bisect_left(self.seek_list, item)
        if self.seek_list[seek_pos] != item:
            raise KeyError(item)

        seek = self.seek_list[seek_pos]
        amount = self.seek_list[seek_pos+1]-seek
